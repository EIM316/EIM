    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const studentRes = await fetch(
          `/api/student/details?student_id=${student_id}`
        );
        const studentJson = await studentRes.json();

        const classId = searchParams.get("class_id");
        if (classId) {
          const classRes = await fetch(`/api/class/info?class_id=${classId}`);
  ...
  try {
    ...
    reader.onloadend = function () {
      ...
      if (window.Android?.saveBase64ToDownloads) {
        ...
        a.href = url;
        a.download = `student_${student_id}_record.csv`;
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col items-center pb-12 relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <h1 className="text-lg font-semibold">
              {classInfo?.class_name || "Student Record"}
            </h1>
      ...
      <section className="w-[90%] max-w-3xl mt-6">
        <div className="bg-white border-2 border-[#7b2020] rounded-xl p-5 shadow-lg">
      ...
      <section className="w-[90%] max-w-4xl mt-6">
        <h2 className="text-[#7b2020] font-bold mb-2">CLASS MODE GAME HISTORY</h2>
