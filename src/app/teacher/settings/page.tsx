...
export default function TeacherSettingsPage() {
  ...
  useEffect(() => {
    ...
    const fetchTeacher = async () => {
      try {
        const res = await fetch(`/api/teacher/get?id_number=${id_number}`);
        ...
        if (res.ok && data.success) {
          ...
          setAvatar(teacher.avatar || "/resources/icons/teach1.jpg");
        } else {
          Swal.fire("Error", data.error || "Failed to load teacher info.", "error");
          router.push("/");
        }
      } catch (err) {
      ...
      <header className="w-full bg-[#7b2020] text-white flex items-center justify-between px-4 py-3 shadow-md fixed top-0 left-0 z-50">
        <div className="flex items-center gap-3">
          ...
          <span className="font-semibold text-lg">
            Prof. {formData.first_name?.toUpperCase()}
          </span>
        </div>
        <LogOut
