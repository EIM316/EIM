...
export default function ChallengeModePage() {
  ...
  const handleAnswer = (choice: "A" | "B" | "C" | "D") => {
    ...
    if (correct) {
      const perQ = Math.floor((settings?.total_points ?? 100) / questions.length);
      setPoints((p) => p + perQ);
      setScore((s) => s + 1);
    }
  };
  ...
  const showResults = () => {
    ...
    Swal.fire({
      ...
      html: `<b>${score}</b> / ${questions.length} correct<br/><span style="color:#548E28;font-size:20px;">Points: ${points}</span>`,
      confirmButtonText: "Back to Menu",
      confirmButtonColor: "#548E28",
    }).then(() => router.push("/student/play"));
  };
  ...
  const isLowTime = totalProgress >= 90; // last 10%
  ...
  return (
    <div className="flex flex-col items-center min-h-screen bg-white text-black">
      ...
      <header className="w-full bg-[#548E28] text-white px-4 py-3 shadow-md">
        ...
        {/* Progress Bar with shake and color */}
      ...
      <main className="w-full max-w-xl px-4 text-center mt-6">
        ...
        <div className="flex flex-col items-center mt-8">
          <button
            ...
            disabled={usedHints >= (settings?.hints_per_student ?? 0)}
            className={`flex items-center gap-3 px-6 py-3 rounded-full font-semibold transition ${
              usedHints >= (settings?.hints_per_student ?? 0)
